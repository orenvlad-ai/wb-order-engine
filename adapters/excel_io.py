...
def generate_input_template() -> io.BytesIO:
    ...
    # Новый лист: Инструкция к заполнению
    ws_instr = wb.create_sheet("Инструкция к заполнению")
    ws_instr.append(["Инструкция к заполнению"])
    ws_instr["A1"].font = _BOLD

    instr_text = [
        "",
        "Вкладка «Ввод данных»",
        "Артикул — точное имя SKU.",
        "Остаток ФФ — текущий остаток на ФФ плюс то, что уже в пути ФФ→МП на момент заполнения таблицы.",
        "Остаток МП — общее количество товара на всех складах МП (суммарно по артикулам).",
        "План, шт/день — значение берём из таблицы СТБ.",
        "Неснижаемый остаток ФФ и Неснижаемый остаток МП — заполняются автоматически формулой. Сделай одну формулу и протяни вниз, чтобы рассчитать все строки.",
        "",
        "Вкладка «Товары в пути»",
        "Здесь указываем все поставки по каждому артикулу, которые уже едут из Китая или находятся в производстве.",
        "Если по одному артикулу несколько партий — пиши их в столбик, каждая партия отдельной строкой: сначала те, что уже в пути, затем — находящиеся в производстве.",
        "Артикул — тот же, что во «Ввод данных».",
        "Кол-во — количество товара в партии.",
        "План. приб. на ФФ — дата прибытия партии на ФФ.",
        "",
        "Коротко:",
        "вводим все актуальные артикулы и остатки, добавляем все поставки построчно, формулы протягиваем вниз — готово к расчёту."
    ]

    for line in instr_text:
        ws_instr.append([line])

    # Автоширина и перенос строк
    for col in ws_instr.columns:
        max_length = max(len(str(cell.value or "")) for cell in col)
        ws_instr.column_dimensions[col[0].column_letter].width = min(max(max_length + 2, 40), 120)
